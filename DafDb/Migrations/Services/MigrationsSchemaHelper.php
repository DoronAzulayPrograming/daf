...
trait MigrationsSchemaHelper
{
    ...
    {
        $mRepo = new MigrationsDbSet($db);
        
        $mapper = new \DafDb\Migrations\Mapper\AttributeToBuilderMapper();
        $t = $mapper->BuildTable($mRepo->GetTableInfo()->Name, $mRepo->GetTableInfo()->ModelClass);

        $provider = $db->GetSqlProvider();
        foreach ($provider->CompileCreateTable($t) as $sql) {
            $db->Execute(new SqlCommand($sql));
        }
    }
