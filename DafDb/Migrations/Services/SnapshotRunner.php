...
final class SnapshotRunner extends SnapshotBuilder {
    ...
    public function ApplyChanges(array $actions = []){
        foreach ($actions as $action) {
            switch ($action['type']) {
                case 'create_table':
                    $name = $action['payload']['name'];
                    $build = $action['payload']['build'];
                    $this->CreateTable($name, $build);
                    break;
                ...
                case 'alter_table':
                    $name = $action['payload']['name'];
                    $alter = $action['payload']['alter'];
                    $this->AlterTable($name, $alter);
                    break;

                case 'raw_sql':
                    // intentionally ignored in snapshots
                    break;
            }
        }
    }
