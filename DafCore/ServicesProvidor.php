...
class ServicesProvidor implements IServicesProvidor, IServicesForCallback {
    ...
    public function AddSingletonInterfaceAndClass(string $serviceName, string $className) : IServicesProvidor {

        $this->container->AddSingleton($serviceName, function($c) use ($className) {
            $vars = $this->GetServicesForCallback($className);
